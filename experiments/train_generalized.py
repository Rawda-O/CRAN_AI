...
from cran.policies.generalized_policy import GeneralizedDFPolicy
...
def main():
    ...
    ap.add_argument("--configs", nargs="+", default=["configs/base.yaml","configs/channel.yaml","configs/df.yaml","configs/generalized.yaml","configs/robust.yaml","configs/multi_objective.yaml","configs/risk.yaml","configs/safety.yaml"])
    ap.add_argument("--out", default="runs/train_generalized_df")
    ...
    # generalized input: 8 gains + (tau, Ps_max, Pr_max)
    in_dim = 11
    model = GeneralizedDFPolicy(in_dim=in_dim)
    info = count_parameters(model)
    logger.info(f"GeneralizedDFPolicy params: {info} size~{model_size_mb(model):.2f} MB")

    df_cfg = cfg["df"]
    # DFParams uses tau; for generalized training we sample tau per batch, so we set a nominal tau here.
    ...
    feat_cfg = FeatureConfig(include_system=True)
    loader_cfg = LoaderConfig(steps_per_epoch=args.steps_per_epoch, generalized=True, robust=robust_on)

    gcfg = cfg["generalized"]
    sampler = SystemSampler(tuple(gcfg["tau_range"]), tuple(gcfg["ps_max_range"]), tuple(gcfg["pr_max_range"]))

    # budgets dict only provides default fallback
    budgets = {"Ps_max": df_cfg["power"]["Ps_max"], "Pr_max": df_cfg["power"]["Pr_max"], "tau": df_cfg["tau"]}
    train_iter = make_epoch_iterator(gen, feat_cfg, loader_cfg, budgets, sampler=sampler)
    ...
    def step(model, batch, device):
        # Update dfp.tau per batch by reconstructing DFParams (clean and explicit)
        # This is important for generalized training correctness.
        tau_b = float(batch["tau"][0].item())
        dfp_b = DFParams(tau=tau_b, noise_power=dfp.noise_power, Pp=dfp.Pp, Rp0=dfp.Rp0)
        return df_step_fn(model, batch, device, dfp=dfp_b, w=w, risk=risk, cfg=scfg)
    ...
    write_markdown_report(apaths.root, "CRAN-AI Generalized DF Training Run", {
        "device": str(device),
        "epochs": args.epochs,
        "steps_per_epoch": args.steps_per_epoch,
        "robust_pairing": robust_on,
        "safety_layer": scfg.safety,
        "include_system_params": True,
    })
