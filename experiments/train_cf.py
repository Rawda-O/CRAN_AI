...
from cran.learning.steps import cf_step_fn, StepConfig
...
def main():
    ...
    ap.add_argument("--configs", nargs="+", default=["configs/base.yaml","configs/channel.yaml","configs/cf.yaml","configs/robust.yaml","configs/multi_objective.yaml","configs/risk.yaml","configs/safety.yaml"])
    ap.add_argument("--out", default="runs/train_cf")
    ...
    in_dim = 8
    model = CFPolicy(in_dim=in_dim)
    info = count_parameters(model)
    logger.info(f"CFPolicy params: {info} size~{model_size_mb(model):.2f} MB")
    ...
    budgets = {"Ps_max": cf_cfg["power"]["Ps_max"], "Pr_max": cf_cfg["power"]["Pr_max"], "tau": cf_cfg["tau"]}
    ...
    scfg = StepConfig(scheme="cf", safety=cfg.get("safety",{}).get("enable", True),
                      rs_threshold=cfg["multi_objective"]["outage"]["Rs_threshold"],
                      outage_k=cfg["multi_objective"]["outage"]["k"])

    def step(model, batch, device):
        return cf_step_fn(model, batch, device, cfp=cfp, w=w, risk=risk, cfg=scfg)
    ...
    write_markdown_report(apaths.root, "CRAN-AI CF Training Run", {
        "device": str(device),
        "epochs": args.epochs,
